...
{
    if (header->tag != PitmDocument::BinaryFormatTag || header->version != 1u)
        return false;
...
int Base::reserveSpace(uint dataSize, int posInTable, uint numItems, bool replace)
{
    ...
    if (size + dataSize >= Value::MaxSize) {
        qWarning("Pitm: Document too large to store in data structure %d %d %d", (uint)size, dataSize, Value::MaxSize);
        return 0;
    }
...
bool Value::isValid(const Base *b) const
{
    ...
    if (s < 0 || s > (int)b->tableOffset - offset)
        return false;
    if (type == PitmValue::Array)
        return static_cast<Array *>(base(b))->isValid(s);
    if (type == PitmValue::Object)
...
/*!
    \internal
 */
int Value::requiredStorage(PitmValue &v, bool *compressed)
{
    *compressed = false;
    switch (v.t) {
    case PitmValue::Double:
        if (PitmPrivate::compressedNumber(v.dbl) != INT_MAX) {
            *compressed = true;
            return 0;
        }
    ...
    case PitmValue::String: {
        ...
        *compressed = PitmPrivate::useCompressed(s);
        return PitmPrivate::qStringSize(s, *compressed);
    }
    case PitmValue::Array:
    case PitmValue::Object:
        if (v.d && v.d->compactionCounter) {
            v.detach();
            v.d->compact();
            v.base = static_cast<PitmPrivate::Base *>(v.d->header->root());
        }
        return v.base ? v.base->size : sizeof(PitmPrivate::Base);
...
/*!
    \internal
 */
uint Value::valueToStore(const PitmValue &v, uint offset)
{
    switch (v.t) {
    case PitmValue::Undefined:
    case PitmValue::Null:
    ...
    case PitmValue::Bool:
        return v.b;
    case PitmValue::Double: {
        int c = PitmPrivate::compressedNumber(v.dbl);
        if (c != INT_MAX)
            return c;
    }
        Q_FALLTHROUGH();
...
/*!
    \internal
 */
void Value::copyData(const PitmValue &v, char *dest, bool compressed)
{
    switch (v.t) {
    case PitmValue::Double:
        if (!compressed) {
    ...
    case PitmValue::String: {
        ...
        PitmPrivate::copyString(dest, str, compressed);
        break;
    }
    case PitmValue::Array:
    case PitmValue::Object: {
        const PitmPrivate::Base *b = v.base;
        if (!b)
            b = (v.t == PitmValue::Array ? &emptyArray : &emptyObject);
